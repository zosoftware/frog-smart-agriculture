          ...
          <div style="display: flex; justify-content: left">
            <div class="control-wrapper">
              <div
                ...
                @mousedown="ptzDirection(0, 1)"
                @mouseup="ptzDirection(0, 0)"
              >
              ...
              <div
                style="
                  ...
                  width: 100px;
                "
                @mousedown="ptzScale(1)"
                @mouseup="ptzScale(0)"
              >
                <svg-icon icon-class="big" style="font-size: 30px" />
                <span style="line-height: 30px; color: #999; margin-left: 6px"
                  >放大</span
                >
              </div>
              <div
              ...
              <el-row type="flex" align="middle" class="ctrl ctrl-btn">
                ...
                >
                  ...
                  <el-slider
                    ...
                    style="width: 100px; margin: 0 30px 0 30px"
                    @change="changeVolume"
                  />
                  <el-button
  ...
  methods: {
    ...
    sendDevicePush: function () {
      ...
      startPlay(this.deviceId, this.channelId).then((response) => {
        ...
        console.log('开始播放12222：' + this.deviceId + ' : ' + this.channelId);
      });
    ...
    close: function () {
      ...
      if (this.streamId && this.playing) {
        stopPlay(this.deviceId, this.channelId, this.streamId).then(
          (response) => {
    ...
    loadDevRecord() {
      ...
      if (this.deviceId && this.channelId) {
        const date = this.queryDate
          ? new Date(this.queryDate).getTime()
        ...
        getDevRecord(this.deviceId, this.channelId, query)
          .then((res) => {
    ...
    playbackstart(playTimes) {
      if (this.deviceId && this.channelId) {
        ...
        this.pausing = false;
        if (this.url.ssrc) {
          playbackStop(this.deviceId, this.channelId, this.url.ssrc)
            .then((res) => {
              const query = {
    ...
    playNext(index) {
      this.playerLoading = true;
      this.url.playurl = '';
      playbackStop(this.deviceId, this.channelId)
        .then((res) => {
          this.playing = false;
          this.url.ssrc = '';
        })
    ...
    seekPlay(s) {
      ...
      if (this.url.ssrc && this.playing) {
        ...
        playbackSeek(this.deviceId, this.channelId, this.streamId, query).then(
          (res) => {
    ...
    pause() {
      if (this.url.ssrc && this.playing) {
        if (this.videoVod) {
          playbackPause(this.deviceId, this.channelId, this.streamId).then(
            (res) => {
              if (res.code === 1) {
                this.pausing = true;
    ...
    resume() {
      if (this.url.ssrc) {
        if (this.pausing) {
          playbackReplay(this.deviceId, this.channelId, this.streamId).then(
            (res) => {
